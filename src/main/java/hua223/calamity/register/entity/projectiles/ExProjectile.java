...
public abstract sealed class ExProjectile extends BaseProjectile
    ...
    public static void create(Player player, Level level) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (projectile != null) {
                projectile.source = CalamityDamageSource.source(DamageTypes.GENERIC, projectile, player)
                    .addDamageTag(DamageTypeTags.IS_PROJECTILE, DamageTypeTags.BYPASSES_ARMOR, DamageTypeTags.BYPASSES_INVULNERABILITY);
    ...
    public static class Render extends EntityRenderer<ExProjectile> {
        ...
        @Override
        public @NotNull ResourceLocation getTextureLocation(ExProjectile excelsusBlue) {
