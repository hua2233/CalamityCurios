...
public enum CalamityItems implements EnumRegister<Item>/*, IExtensibleEnum */ {
    ...
    ETHEREAL_CORE("ethereal_core", () -> new MagicItem(UNCOMMON_ONE, 1)),

    PHANTOM_HEART("phantom_heart", () -> new MagicItem(EPIC_ONE, 2)),
    ...
    MUSHROOM_PLASMA_ROOT("mushroom_plasma_root", () -> new RageItem(RARE_ONE, 0)),

    INFERNAL_BLOOD("infernal_blood", () -> new RageItem(UNCOMMON_ONE, 1)),

    LIGHTNING_CONTAINER("red_lightning_container", () -> new RageItem(EPIC_ONE, 2)),
    ...
    SANGUINE_TANGERINE("sanguine_tangerine", () -> new LifeFruit(Rarity.UNCOMMON, 1)),

    MIRACLE_FRUIT("miracle_fruit", () -> new LifeFruit(Rarity.RARE, 2)),

    TAINTED_CLOUDBERRY("tainted_cloudberry", () -> new LifeFruit(Rarity.EPIC, 3)),

    SACRED_STRAWBERRY("sacred_strawberry", () -> new LifeFruit(RegisterList.CALAMITY, 4)),
    ...
    BOUNDING("bounding", () -> new BoundingPotion(getPotionProperties(Rarity.RARE), "bounding", () ->
    ...
    OMNISCIENCE("omniscience", () -> new OmnisciencePotion(getPotionProperties(Rarity.EPIC), "omniscience", () ->
    ...
    DEMON_SHADE_HELMET("demon_shade_helmet", () -> new DemonShade(ArmorItem.Type.HELMET)),

    DEMON_SHADE_CHEST("demon_shade_chest", () -> new DemonShade(ArmorItem.Type.CHESTPLATE)),

    DEMON_SHADE_LEGGINGS("demon_shade_leggings", () -> new DemonShade(ArmorItem.Type.LEGGINGS)),
    ...
    EXCELSUS("excelsus", () -> new Excelsus(14, -2f, EPIC_ONE)),

    ATARAXIA("ataraxia", () -> new Ataraxia(11, -2f, EPIC_ONE)),
    ...
    DREAM_CATCHER("dream_catcher", () -> new DreamCatcher(EPIC_ONE)),

    ANTI_VOID("anti_void", () -> new AntiVoid(EPIC_ONE)),

    STARLESS_NIGHT("starless_night", () -> new StarlessNight(RARE_ONE)),

    XYTHERON("xytheron", Xytheron::new),

    ZENITH("zenith", () -> new Zenith(10, -2f,
        new Item.Properties().defaultDurability(-1).rarity(Rarity.EPIC)));

    private final RegistryObject<Item> value;

    CalamityItems(String id, Supplier<Item> curio) {
        this.value = ITEMS.register(id, curio);
        if (fillCurios) curiosIndex++;
    }

    CalamityItems(Supplier<Item> curio, String id) {
        if (!fillCurios) throw new IllegalStateException("Already on the item list");
        fillCurios = false;
        this.value = ITEMS.register(id, curio);
    }
