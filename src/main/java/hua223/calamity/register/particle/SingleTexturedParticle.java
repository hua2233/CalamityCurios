...
public abstract class SingleTexturedParticle extends TextureSheetParticle {
    ...
    protected void resetVertexData(float x, float y, float z, float height, float width, @NotNull Quaternionf quaternion) {
        vertex[0].set(-height, -width, 0);
        vertex[1].set(-height, width, 0);
        vertex[2].set(height, width, 0);
        vertex[3].set(height, -width, 0);

        vertex[0].rotate(quaternion);
        vertex[1].rotate(quaternion);
        vertex[2].rotate(quaternion);
        vertex[3].rotate(quaternion);
    ...
    protected void byVertexDataBuild(VertexConsumer buffer, Vector4i color) {
        byVertexDataBuild(buffer, color.x(), color.y(), color.z(), color.w());
    }
    ...
    protected static abstract class SingleTexturedProvider<T extends ParticleOptions> implements ParticleProvider<T> {
        ...
        @Override
        public @Nullable Particle createParticle(@NotNull T type, @NotNull ClientLevel clientLevel, double v, double v1, double v2, double v3, double v4, double v5) {
