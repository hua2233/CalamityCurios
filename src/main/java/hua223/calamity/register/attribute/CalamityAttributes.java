...
public enum CalamityAttributes implements EnumRegister<Attribute> {
    ...
    CalamityAttributes(String name, double defaultValue, double min, double max) {
        this(name, new RangedAttribute("attribute.calamity_curios." + name, defaultValue, min, max));
    }

    CalamityAttributes(String name, Attribute attribute) {
        this.value = ATTRIBUTES.register(name, () -> attribute);
    }
