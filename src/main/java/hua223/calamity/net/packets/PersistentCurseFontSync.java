...
public class PersistentCurseFontSync extends DataPack {
    ...
    private record FontInfo(Item item, boolean gradual, int start, int end, int cycle) {
        ...
        public static FontInfo deserialization(FriendlyByteBuf buf) {
            return new FontInfo(buf.readById(BuiltInRegistries.ITEM), buf.readBoolean(), buf.readInt(), buf.readInt(), buf.readInt());
        }
    }
}
